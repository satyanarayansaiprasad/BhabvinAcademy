    ...
    <>
              ...
              {filteredStudents.map((student) => {
                ...
                return (
                  <div key={student.studentId}>
                    {/* Student Summary Row */}
                    <div className="w-full flex items-center gap-4 px-5 py-4 hover:bg-zinc-50 transition-colors">
                      {/* Avatar */}
                      <button onClick={() => setExpandedStudent(isExpanded ? null : student.studentId)} className="flex flex-1 items-center gap-4 text-left min-w-0">
                        ...
                        <div className="hidden md:flex items-center gap-3 w-36 shrink-0">
                          <div className="flex-1 h-1.5 bg-zinc-100 rounded-full overflow-hidden">
                            <div className="h-full bg-[#0067b8] rounded-full" style={{ width: `${avgProgress}%` }} />
                          </div>
                              ...
                              <div key={course.courseId} className="flex items-center gap-4 bg-white rounded-lg border border-zinc-100 px-4 py-3">
                                ...
                                <div className="flex items-center gap-3 w-44 shrink-0">
                                  <div className="flex-1 h-2 bg-zinc-100 rounded-full overflow-hidden">
                                    <div
                                      className={`h-full rounded-full ${course.isCompleted ? "bg-emerald-500" : "bg-[#0067b8]"}`}
                                      style={{ width: `${course.completionPercentage}%` }}
                                    />
                                  </div>
                                  <span className="text-xs font-bold text-zinc-500 w-9 text-right">{course.completionPercentage}%</span>
                                </div>
                              </div>
                            ))}
                          </div>
      ...
      {deletingStudent && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[200] flex items-center justify-center p-6">
          <div className="bg-white rounded-xl p-8 max-w-sm w-full shadow-2xl text-center border border-zinc-200">
            <div className="w-14 h-14 bg-red-50 text-red-500 rounded-full flex items-center justify-center mx-auto mb-5">
              <AlertCircle className="w-7 h-7" />
            </div>
            <h2 className="text-xl font-bold text-zinc-900 mb-2">Delete Student?</h2>
            <p className="text-zinc-500 text-sm mb-1">
              <span className="font-semibold text-zinc-700">{deletingStudent.userFullName || deletingStudent.userName}</span>
            </p>
            <p className="text-zinc-400 text-xs mb-7">
              This will permanently delete the student's account, course enrollments, and all progress data. This cannot be undone.
            </p>
            <div className="flex flex-col gap-2">
              <button
                onClick={handleDeleteStudent}
                disabled={isDeleting}
                className="w-full h-11 bg-red-600 hover:bg-red-700 text-white rounded-lg font-semibold text-sm flex items-center justify-center gap-2 transition-colors"
              >
                {isDeleting ? <RefreshCw className="h-4 w-4 animate-spin" /> : <><Trash2 className="h-4 w-4" /> Delete Permanently</>}
              </button>
              <button
                onClick={() => setDeletingStudent(null)}
                className="w-full h-11 border border-zinc-200 rounded-lg text-zinc-600 font-semibold text-sm hover:bg-zinc-50 transition-colors"
              >
                Cancel
              </button>
            </div>
